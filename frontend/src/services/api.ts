...
// SubGoal endpoints
export const addSubGoalApi = async (subGoal: FormData | any) => {
  const response = await axios.post(`${API_BASE_URL}/goals/sub-goals/`, subGoal, {
    headers: { ...getAuthHeader() },
  });
  return response.data;
};
...
export const updateSubGoalApi = async (subGoal: any) => {
  const response = await axios.put(`${API_BASE_URL}/goals/sub-goals/${subGoal.id}/`, subGoal, {
    headers: getAuthHeader(),
  });
  return response.data;
};
